...
class UniversalIntentService {
  ...
  final IntentExtractionService _intentExtraction = IntentExtractionService();
  ...
  // Process user intent and find matches - NOW USING INTENT-BASED MATCHING
  Future<Map<String, dynamic>> processIntentAndMatch(String userInput) async {
    try {
      print('DEBUG: Processing user input with new intent-based system: $userInput');

      // Use the new intent extraction service for pure intent-based matching
      final result = await _intentExtraction.processUserInput(userInput);

      if (result['success'] == true) {
        print('DEBUG: Intent extracted - Intent: ${result['intent_extracted']['intent']}, Target: ${result['intent_extracted']['target_intent']}');
        print('DEBUG: Found ${result['matches']?.length ?? 0} matches');

        return {
          'success': true,
          'intent': result['intent'],
          'matches': result['matches'],
          'intent_extracted': result['intent_extracted'],
        };
      } else {
        return result;
      }
  ...
  // Process intent without storing - for clarification flow
  Future<Map<String, dynamic>> processIntentOnly(String userInput) async {
    try {
      ...
      final intentData = _parseGeminiResponse(geminiResponse, userInput);

      // Return intent data without storing
      return {
        'success': true,
  ...
  // Store processed intent and find matches
  Future<Map<String, dynamic>> storeAndMatch(Map<String, dynamic> intentData, String userId) async {
    try {
      ...
      print('Error storing intent and finding matches: $e');
      return {
        'success': false,
        'error': e.toString(),
      };
...
''';
  ...
  Map<String, dynamic> _parseGeminiResponse(String response, String userInput) {
    try {
      ...
      return {
        ...
        'user_role': 'USER',
        'match_role': 'MATCH',
        'title': parsed['title'] ?? _generateTitleFromInput(userInput),
      ...
      return {
        ...
        'user_role': 'USER',
        'match_role': 'MATCH',
        'title': _generateTitleFromInput(userInput),
  ...
  Future<List<Map<String, dynamic>>> _findComplementaryMatches(
    ...
    try {
      ...
      if (currentUserId == null) {
        print('ERROR: User not authenticated for finding matches');
        return [];
      }
      ...
      try {
        ...
        print('WARNING: Failed to get user location: $e');
      ...
      if (searchEmbedding.isEmpty) {
        print('WARNING: Failed to generate search embedding');
        return [];
      }
      ...
      // Get potential matches with error handling
      QuerySnapshot querySnapshot;
      try {
        querySnapshot = await query.limit(200).get();
      } catch (e) {
        print('ERROR: Failed to query Firestore: $e');
        return [];
      }
      ...
      for (var doc in querySnapshot.docs) {
        ...
        try {
          ...
          if (storedEmbedding.isNotEmpty && searchEmbedding.isNotEmpty &&
              storedEmbedding.length == searchEmbedding.length) {
            matchScore = _geminiService.calculateSimilarity(searchEmbedding, storedEmbedding);

            // Validate similarity score
            if (!matchScore.isFinite || matchScore < 0 || matchScore > 1) {
              print('WARNING: Invalid similarity score: $matchScore');
              matchScore = 0.0;
            }
          }
        } catch (e) {
          print('ERROR: Similarity calculation failed: $e');
          matchScore = 0.0;
        }
  ...
  Future<List<Map<String, dynamic>>> getUserIntents(String userId) async {
    try {
      // Query with composite index for optimal performance
  ...
  String _generateTitleFromInput(String userInput) {
    // Clean and truncate the input
    String title = userInput.trim();
    ...
    // Truncate if too long (keep it reasonable for UI)
    if (title.length > 50) {
