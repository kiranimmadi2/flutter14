    ...
    try {
      ...
      if (position != null) {
        ...
        if (addressData != null && mounted) {
          final locationString = addressData['display'] ?? addressData['city'] ?? 'Location detected';
          setState(() {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Could not detect city name'),
                backgroundColor: Colors.orange,
              ),
            );
          }
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Location permission denied or location services disabled'),
              backgroundColor: Colors.red,
            ),
          );
        }
